        ...
        try:
            ...
            if path == "/export-markdown":
                if self.server.export_service is None:
                    self._send_json(ExportResult(ExportStatus.FAILED, str(payload.get("session_id", "")), "Markdown export unavailable").to_dict(), status=400)
                    return
                session = SessionRef(session_id=str(payload.get("session_id", "")), title=str(payload.get("title", "")))
            ...
            if path == "/export-markdown":
                result = ExportResult(ExportStatus.FAILED, str(payload.get("session_id", "")) if "payload" in locals() else "", str(exc))
